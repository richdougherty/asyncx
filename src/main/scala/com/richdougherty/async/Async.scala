...
sealed trait Async[+A] {
  ...
  //def asyncMap[B]: AsyncFunc[AsyncFunc[A,B],Async[B]] = AsyncFunc.TrivialLifted(f => Map(this, f))
  //def asyncFlatMap[B]: AsyncFunc[AsyncFunc[A,Async[B]],Async[B]] = AsyncFunc.TrivialLifted(f => FlatMap(this, f))
  //def flatten[B](implicit ev: A <:< Async[B]): Async[B] = Flatten(this.asInstanceOf[Async[Async[B]]] /* FIXME: eliminate cast? */)
}
